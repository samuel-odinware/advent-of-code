...
impl<'a> Game<'a> {
    fn maximum_cubes_needed(&self, map: &Vec<&str>) -> Option<u32> {
        self.rounds
            .iter()
            .any(|round| {
            ...
            .then_some(
                self.id
                    .parse::<u32>()
                    .expect("The game id should a parsable u32."),
            )
    }
}
...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    let map = Vec::from(["red", "green", "blue"]);
    let games = parse_games(input).expect("Should parse games.");

    Ok(games
        .1
        .iter()
        .filter_map(|game| game.maximum_cubes_needed(&map))
        .sum::<u32>()
        .to_string())
}
